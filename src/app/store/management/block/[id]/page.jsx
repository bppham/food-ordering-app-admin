"use client";
import { getInformation, unblockStore } from "../../../../../api/store";
...
const Page = () => {
  ...
  const handleReturn = () => {
    router.push(`/store/management/block`);
  };

  const handleUnBlock = async (storeId) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (res.success) {
          ...
          Swal.fire({
            title: "Thành công!",
            text: "Cửa hàng hoạt động bình thường!",
            icon: "success",
  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      <div className="flex justify-between items-center p-4 mt-4">
        ...
        <button
          onClick={() => handleUnBlock(store._id)}
          className="flex gap-2 items-center justify-center bg-green-600 hover:bg-green-500 text-white text-xs font-semibold w-[150px] py-2 rounded-lg transition-colors"
        >
          <FaUnlock />
          Mở khóa cửa hàng
        </button>
